...
object ConsoleProducer {
  ...
  def main(args: Array[String]) {
    ...
    val fileOpt = parser.accepts("url", "URL of the input stream")
                            .withRequiredArg
                            .describedAs("input_stream")
                            .ofType(classOf[java.lang.String])
    val urlFactoryOpt = parser.accepts("url-factory", "The class name of url factory to use.")
                            .withRequiredArg
                            .describedAs("factory_class")
                            .ofType(classOf[java.lang.String])
    val idleTimeOpt = parser.accepts("idle-time", "Wait sleep time before quit. The value is given in ms.")
                            .withRequiredArg
                            .describedAs("sleep_time")
                            .ofType(classOf[java.lang.Long])
                            .defaultsTo(60000)
    ...
    if (urlFactoryClass != null) {
      URL.setURLStreamHandlerFactory(Class.forName(urlFactoryClass).newInstance().asInstanceOf[URLStreamHandlerFactory])
    }
